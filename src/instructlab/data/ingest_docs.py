...
def ingest_docs(
  ...
  pipeline = _create_pipeline(vectordb_type=vectordb_type, vectordb_uri=vectordb_uri, embedding_model=embedding_model)
...
def _create_pipeline(
  ...
  pipeline.add_component(instance=DocumentSplitter(split_by="word", split_length=150, split_overlap=50),
                          name="document_splitter")
  # TODO make this more generic
  pipeline.add_component(instance=SentenceTransformersDocumentEmbedder(model=embedding_model), name="document_embedder")
  pipeline.add_component(instance=DocumentWriter(
    _document_store(vectordb_type=vectordb_type, vectordb_uri=vectordb_uri)), name="document_writer")
  return pipeline
...
def _ingest_docs(pipeline, output_dir):
  ...
  logger.info(f"document_writer.documents_written: {ingestion_results['document_writer']['documents_written']}")
