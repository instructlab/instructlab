...
def lookup_device(device: torch.device) -> "AbstractTorchDevice":
    ...
    plugin_class = base._plugin_registry.get(device.type)
    ...
    if typing.TYPE_CHECKING:
        assert not inspect.isabstract(plugin_class)
        assert callable(plugin_class)

    return plugin_class(device)
...
class AbstractTorchDevice(metaclass=abc.ABCMeta):
    ...
    def is_supported(cls) -> bool:
        ...
        mean that the device is available.
        """
    ...
    def __bool__(self) -> bool:
        """True if device and its modele are available"""
    ...
    def load_module(self) -> types.ModuleType:
        ...
        PyTorch is inconsistent. Some devices have both torch.{TYPE} and
        torch.backend.{TYPE}.
        """
...
class AbstractCudaDevice(AbstractTorchDevice):
    ...
    def is_bf16_supported(self) -> bool:
        ...
        return self.mod.is_bf16_supported()
...
class HabanaDevice(AbstractTorchDevice):
    ...
    def load_module(self) -> types.ModuleType:
        # this import registers 'torch.hpu', 'hpu' device, and 'hpu_backend'
        # for dynamo (torch.compile).
        # pylint: disable=import-error
        # Third Party
        import habana_frameworks.torch.core as htcore
        import habana_frameworks.torch.dynamo.compile_backend.backends as hpu_backends
