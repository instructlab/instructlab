...
def linux_test(
    ...
    def test_model(model: Path):
        def response(user, create_params):
        ...
        create_params["model"] = model  # mandatory, but not used
        ...
        try:
            ...
            backend_instance = backends.select_backend(
                logger, ctx.obj.config.serve, model
            )
            ...
            logger.debug("%s", api_base)
            client = OpenAI(
                base_url=api_base,
                api_key=ctx.params["api_key"],
            )
            for d in ds:
                res[d["user"]][model] = response(d["user"], create_params)
        except Exception as exc:
            raise exc
        finally:
            if backend_instance:
