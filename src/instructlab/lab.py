...
def train(
    ...
    if not utils.is_macos_with_m_chip():
        ...
        os.makedirs(final_results_dir := training_results_dir + "/final", exist_ok=True)
        for f in [
            glob(training_results_dir + "/checkpoint-*/added_tokens.json"),
            glob(training_results_dir + "/checkpoint-*/special_tokens_map.json"),
            glob(training_results_dir + "/checkpoint-*/tokenizer.json"),
            glob(training_results_dir + "/checkpoint-*/tokenizer.model"),
            glob(training_results_dir + "/checkpoint-*/tokenizer_config.json"),
            glob(training_results_dir + "/merged_model/config.json"),
            glob(training_results_dir + "/merged_model/generation_config.json"),
        ]:
            shutil.copy(f[0], final_results_dir)
            print("Copied ", f[0], "to ", final_results_dir)
