...
def list_datasets(dataset_dirs):
    ...
    headers = ["Model", "Dataset", "Created At", "File size"]
    ...
    for item in data:
        filename = item[0]
        model_name = extract_model_and_timestamp(os.path.basename(filename))
        entry = [model_name, filename, item[1], item[2]]
        run_id = item[3]
        grouped_data[run_id].append(entry)

    for idx, run_id in enumerate(
            sorted(grouped_data.keys(), reverse=True)
        ):
        click.echo(f"{os.linesep if idx > 0 else ''}{'Uncategorized datasets' if run_id == '' else f'Run from {run_id}'}")
