...
def test_process_record_remove(cli_runner: CliRunner):
    ...
    process_registry["33333333-799a-42a7-af02-d0b68fddf19d"] = {
        "pid": 20003,
        "children_pids": [444, 555, 666],
        "type": "Generation",
        "log_file": "/Users/test/.local/share/instructlab/logs/generation/generation-33333333-799a-42a7-af02-d0b68fddf19d.log",
        "start_time": "2025-01-02T21:46:13",
        "status": ILAB_PROCESS_STATUS.DONE.value,
    }
    process_registry["44444444-799a-42a7-af02-d0b68fddf19d"] = {
        "pid": 20004,
        "children_pids": [777, 888, 999],
        "type": "Generation",
        "log_file": "/Users/test/.local/share/instructlab/logs/generation/generation-44444444-799a-42a7-af02-d0b68fddf19d.log",
        "start_time": "2025-01-02T21:46:13",
        "status": ILAB_PROCESS_STATUS.DONE.value,
    }
    ...
    assert "Removed 1 processes" in result.output
    ...
    assert "Are you sure you want to remove the processes?" in result.output
    ...
    assert result.exit_code == 0
    assert (
        "Process with UUID 33333333-799a-42a7-af02-d0b68fddf19d removed"
        in result.output
    )
    assert (
        "Process with UUID 44444444-799a-42a7-af02-d0b68fddf19d removed"
        in result.output
    )
