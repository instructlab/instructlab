...
from huggingface_hub.utils import RevisionNotFoundError, EntryNotFoundError
...
def download(ctx, repository, filename, release, model_dir, hf_endpoint):
    ...
    try:
        ...
        click.secho(
            f"Downloading model failed due to an invalid revision with the following error: {exc}",
            fg="red",
        )
