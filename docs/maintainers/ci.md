...
Unit tests are designed to test specific InstructLab components or features in isolation. In CI, these tests are run on Python 3.12, 3.11 and 3.10 on CPU-only Ubuntu
...
Python 3.12, 3.11 and 3.10 on CPU-only Ubuntu and MacOS runners. New code may or may not need a functional test but should strive
