...
Unit tests are designed to test specific InstructLab components or features in isolation. In CI, these tests are run on Python 3.12, Python 3.11 and Python 3.10 on CPU-only Ubuntu
runners as well as Python 3.12 and Python 3.11 on CPU-only MacOS runners. Generally, new code should be adding or modifying unit tests.
...
Python 3.12, Python 3.11 and Python 3.10 on CPU-only Ubuntu runners as well as Python 3.12 and Python 3.11 on CPU-only MacOS runners. New code may or may not need a functional test but should strive
