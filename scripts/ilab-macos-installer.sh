...
check_python_version() {
    ...
    if [[ "$PYTHON_VERSION" =~ ^3\.1(0|1|2) ]]; then
        echo -e "$INDENT_VARIABLE Detected Python version: $PYTHON_VERSION"
        return 0
    else
        echo -e "$INDENT_VARIABLE Detected Python version: $PYTHON_VERSION (not 3.10, 3.11 or 3.12)"
        exit 1
    fi
}
...
main() {
    ...
    # Try checking python3.12 first (priority), then python 3.11, then python3.10, then python3
