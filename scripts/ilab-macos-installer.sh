...
check_python_version() {
    ...
    if [[ "$PYTHON_VERSION" =~ ^3\.(11|12) ]]; then
        ...
        echo -e "$INDENT_VARIABLE Detected Python version: $PYTHON_VERSION (not 3.11 or 3.12})"
        exit 1
    fi
}
...
main() {
    ...
    # Try checking python3.12 first (priority), then python3.11, then python3
    ...
    if check_python_version python3.12; then
        ...
        echo "Error: Python 3.12 or 3.11 is required but not found."
        exit 1
    fi
