...
def run_training_phase(
    ...
    phased_base_dir = os.path.join(data_path, "instructlab", "phased-resume")
    ...
    if fail_on_phase1 or fail_on_phase2:
        ...
        if fail_on_phase1:
            assert_phase1(result)
        elif fail_on_phase2:
            assert_phase2_resumed(result)
    elif fail_on_mt_bench:
        assert INTENTIONAL_MT_BENCH_FAILURE_MESSAGE in result.output
        assert_phase2_eval_resumed(result)
    else:
...
def assert_phase2_resumed(result):
    assert_phase1(result)
    assert_phase2(result)


def assert_phase2_eval_resumed(result):
    assert_phase2(result)
    assert_phase2_eval(result)
...
def test_phased_training_resume(
    knowledge_data_path, skills_data_path, data_path, config
):
    ...
    run_training_phase(knowledge_data_path, skills_data_path, data_path, config)
