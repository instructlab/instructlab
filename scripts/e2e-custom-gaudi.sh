...
MIXTRAL_SAFETENSOR_REPO="mistralai/Mixtral-8x7B-Instruct-v0.1"
...
test_download() {
    ...
    if [ "$MIXTRAL" -eq 1 ]; then
	    ...
	    ilab model download --repository mistralai/Mixtral-8x7B-Instruct-v0.1 --filename ${MIXTRAL_SAFETENSOR_REPO} --hf-token "${HF_TOKEN}"
    else
...
test_serve() {
    ...
    if [ "${model_type}" == "base" ]; then
        ...
        if [ "${MIXTRAL}" -eq 1 ]; then
            model_path="${CACHE_HOME}/instructlab/models/${MIXTRAL_SAFETENSOR_REPO}"
...
test_generate() {
    ...
    if [ "$MIXTRAL" -eq 1 ]; then
        GENERATE_ARGS+=("--model" "${CACHE_HOME}/instructlab/models/${MIXTRAL_SAFETENSOR_REPO}")
    else
    ...
    ilab --config=src/instructlab/profiles/intel/gaudi/gaudi_3.yaml data generate --sdg-scale-factor ${NUM_INSTRUCTIONS} "${GENERATE_ARGS[@]}"
...
test_train() {
    ...
    if [ "$ACCELERATED_TRAIN" -eq 1 ]; then
        ...
        TRAIN_ARGS=("--pipeline=accelerated" "--model-path=${GRANITE_SAFETENSOR_REPO}" "--data-path=${data}")
	if [ "${BACKEND}" != "vllm" ]; then
    ...
    ilab --config=src/instructlab/profiles/intel/gaudi/gaudi_3.yaml model train "${TRAIN_ARGS[@]}"
...
test_evaluate() {
	...
	local judge_model_path="${CACHE_HOME}/instructlab/models/prometheus-eval/prometheus-8x7b-v2.0"
    ...
    ilab model evaluate --model "${model_path}" --judge-model "${judge_model_path}" --enable-serving-output --benchmark mt_bench
    task Running MT_Bench_Branch
    cd "${DATA_HOME}/instructlab/taxonomy"
    git branch rc
    cd "${DATA_HOME}/instructlab"
    GPUS=$(hl-smi --format=csv,noheader | wc -l)
    ilab model evaluate \
    --model "${model_path}" \
...
test_exec() {
    ...
    test_taxonomy 3
    test_generate
    end_time_generate=$(date +%s%3N)
    duration_generate_ms=$((end_time_generate - start_time_generate))
    
    git config --global --add safe.directory /root/jzhang90/ilab-e2e/instructlab
    test_data_list
    ...
    duration_train_ms=$((end_time_train - start_time_train))
    ...
    start_time_evaluate=$(date +%s%3N)
    if [ "$EVAL" -eq 1 ]; then
        test_evaluate
    fi
    end_time_evaluate=$(date +%s%3N)
    duration_evaluate_ms=$((end_time_evaluate - start_time_evaluate))
    
    end_time_e2e=$(date +%s%3N)
    duration_e2e_ms=$((end_time_e2e - start_time_e2e))
    echo "Evaluate execution time in ms: $duration_evaluate_ms"
    echo "Train execution time in ms: $duration_train_ms"
    echo "Generate execution time in ms: $duration_generate_ms"
    echo "E2E execution time in ms: $duration_e2e_ms"
